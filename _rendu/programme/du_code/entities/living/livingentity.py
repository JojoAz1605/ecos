...
#from du_code.animation import AnimateSprite
...
class LivingEntity(pygame.sprite.Sprite):
    ...
    def can_reproduce(self) -> bool:
        ...
        if self.gender == 1 and self.age >= self.age_mini and self.world.year >= 1 and pygame.sprite.spritecollide(self,
                                                                                                                   entity_group,
                                                                                                                   False) and len(
            self.world.entities[self.type]) < 50:
            for entity in entity_group:
                if self.rect.colliderect(
                        entity.rect) and entity.gender != self.gender and entity.age >= entity.age_mini:
                    return True
        return False
    ...
    def give_birth(self):
        ...
        """
        position_offset = [self.position[0] + self.position[0] % 16, self.position[1] + self.position[
            1] % 16]  # sert pour bien placer le nouveau-né sur la grille
        newChild = LivingEntity(position_offset, self.name + " child", randint(0, 1),
                                self.world)  # créer une nouvelle créature
    ...
    def check_pregnant(self) -> None:
        ...
        if self.pregnant["is_pregnant"] and self.pregnant[
            "time_pregnant"] == self.pregnancy_time:  # si la créature est enceinte et qu'elle est prête à mettre bas
            ...
            "is_pregnant"]:  # si la créature est enceinte, mais qu'elle n'est pas encore prête à mettre bas
